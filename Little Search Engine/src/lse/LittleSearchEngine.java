...
public class LittleSearchEngine {
	...
	throws FileNotFoundException {
		...
		while (sc.hasNext()) {
			String checkKey=getKeyword(sc.next());
			if (checkKey!=null) {
				if (myMap.containsKey(checkKey)) {
					Occurrence oc=myMap.get(checkKey);
					oc.frequency++;
				}
				else {
					Occurrence oc=new Occurrence(docFile,1);
					myMap.put(checkKey,oc);
				}
			}
		}
		return myMap;
	...
	public void mergeKeywords(HashMap<String,Occurrence> kws) {
		...
		for (String k: kws.keySet()) {
			...
			if (keywordsIndex.containsKey(k)) 
				ocList=keywordsIndex.get(k);
			ocList.add(kws.get(k));
			insertLastOccurrence(ocList);
			keywordsIndex.put(k, ocList);
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		...
		while (high>=low) {
			mid=(low+high)/2;
			int theData=occs.get(mid).frequency;
			midList.add(mid);
			
			if (theData==target)
				break;
			else if (theData<target)
				high=mid-1;
			else if (theData>target) {
				low=mid+1;
				if (high<=mid)
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		if (!oc1.isEmpty() && !oc2.isEmpty()) {
			...
			for (int i=0;i<total.size()-1;i++) {
				for (int j=1;j<total.size()-i;j++) {
					if (total.get(j-1).frequency <total.get(j).frequency) {
						Occurrence temp=total.get(j-1);
			...
			for (int i=0;i<total.size()-1;i++) {
				for (int j=i+1;j<total.size();j++) {
					if (total.get(i).document==total.get(j).document)
						total.remove(j);
				}
			}
		}
